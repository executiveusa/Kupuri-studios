...
function ImageModelBadge({
  ...
  useEffect(() => {
    ...
    const loadModelInfo = async () => {
      setIsLoading(true)
      try {
        // Mock PNG metadata loading
        await new Promise((resolve) => setTimeout(resolve, 500))

        // Mock model info
        const mockModels = ['Stable Diffusion v1.5', 'DALL-E 3', 'Midjourney v6', 'SDXL']
        const randomModel = mockModels[Math.floor(Math.random() * mockModels.length)]
        setModelInfo(randomModel)
      } catch (error) {
...
export default function MaterialManager() {
  ...
  // Mock API functions
  const mockBrowseFolderApi = async (path: string = ''): Promise<BrowseResult> => {
  ...
  const mockGetMediaFilesApi = async (path: string): Promise<FileSystemItem[]> => {
    await new Promise((resolve) => setTimeout(resolve, 600))

    const mockMediaFiles: FileSystemItem[] = [
      {
        name: 'concept-art.png',
        path: `${path}/concept-art.png`,
        type: 'image',
        size: 8192000,
        mtime: Date.now() / 1000 - 900,
        is_directory: false,
        is_media: true,
        has_thumbnail: true,
      },
      {
        ...
        path: `${path}/character-design.jpg`,
        type: 'image',
        size: 6144000,
        mtime: Date.now() / 1000 - 1800,
        is_directory: false,
      ...
      {
        name: 'environment-map.jpg',
        path: `${path}/environment-map.jpg`,
        type: 'image',
        size: 10240000,
        mtime: Date.now() / 1000 - 2700,
        is_directory: false,
        is_media: true,
        has_thumbnail: true,
      },
  ...
  const mockGetMyAssetsDirPath = async () => {
  ...
  const mockGetFileServiceUrl = (filePath: string) => {
    // Mock URL for local development
    return `https://via.placeholder.com/400x300/3b82f6/ffffff?text=${encodeURIComponent(filePath.split('/').pop() || 'File')}`
  }

  const mockReadPNGMetadata = async (url: string) => {
  ...
  const mockGetImageDimensions = useCallback(
  ...
  const loadFolder = useCallback(
    async (path: string = '') => {
      ...
      try {
        const result: BrowseResult = await mockBrowseFolderApi(path)
        ...
        if (selectedFolder && selectedFolder.is_directory) {
          try {
            const mediaResult = await mockGetMediaFilesApi(selectedFolder.path)
  ...
  const loadFolderContents = useCallback(async (folderPath: string) => {
    try {
      const result: BrowseResult = await mockBrowseFolderApi(folderPath)
  ...
  const handleFolderClick = useCallback(
    async (folder: FileSystemItem) => {
      if (folder.is_directory) {
        ...
        try {
          const mediaResult = await mockGetMediaFilesApi(folder.path)
          setMediaFiles(mediaResult)
        } catch (err) {
          console.error('Failed to load media files:', err)
          setMediaFiles([])
  ...
  const handleFileClick = useCallback(
    async (file: FileSystemItem) => {
      ...
      if (file.type === 'image') {
        try {
          const dimensions = await mockGetImageDimensions(file.path)
        ...
        if (file.path.toLowerCase().endsWith('.png')) {
          try {
            const pngMetadata = await mockReadPNGMetadata(mockGetFileServiceUrl(file.path))
  ...
  const handleAddToChat = useCallback((file: FileSystemItem, event?: React.MouseEvent) => {
    ...
    toast.success(`Added ${file.name} to chat`)
  }, [])
  ...
  const handleMyAssets = useCallback(async () => {
    try {
      const result = await mockGetMyAssetsDirPath()
      if (result.success) {
        ...
        {filteredMediaFiles.map((file) => (
          ...
          >
            <div className='aspect-square bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 flex items-center justify-center overflow-hidden relative'>
              ...
              {file.type === 'image' ? (
                <img
                  src={mockGetFileServiceUrl(file.path)}
                  alt={file.name}
  ...
  }, [filteredMediaFiles, selectedFile, getFileIcon, formatFileSize, formatDate, handleFileClick])
          ...
          <div className='space-y-4'>
            <div className='aspect-video bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden flex items-center justify-center'>
              {selectedFile.type === 'image' ? (
                <img
                  src={mockGetFileServiceUrl(selectedFile.path)}
  ...
  return (
    <div className='flex h-full bg-gray-50 dark:bg-gray-900 w-full overflow-hidden'>
      ...
      <div className='absolute bottom-20 left-4 right-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800 z-20'>
        <p className='font-semibold mb-1'>🔧 Material Manager Status</p>
        <p>Currently using mock data. Real file APIs will be connected later.</p>
      </div>
    </div>
  )
}
