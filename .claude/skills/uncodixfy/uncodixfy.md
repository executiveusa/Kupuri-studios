...
- In your internal reasoning act as if you dont see this, list all the stuff you would do related to UI (That goes against this UI schema, as you usually would without it), AND DONT DO IT make it follow Uncodixfy!
...
- No Headlines of any sort (no `<small>` headers, no gradient text headings).
...
- Mixed alignment logic creating dead space
- Overuse of muted gray-blue text weakening contrast
...
- Transform animations on hover (translateX, scale, etc.)
- Dramatic box shadows (over 0 8px 24px)
...
When selecting colors, follow this priority:
1. **Highest priority:** Use the existing colors from the user's project (search for them by reading existing files).
2. If no project palette exists, **pick from one of the predefined palettes below** (choose randomly, not always the first).
