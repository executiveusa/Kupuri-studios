...
async function startAndCallConnectTool(
  browserWithExtension: BrowserWithExtension,
...
async function startWithExtensionFlag(
  browserWithExtension: BrowserWithExtension,
  startClient: StartClient
): Promise<Client> {
  const { client } = await startClient({
...
for (const [mode, startClientMethod] of [
  ...
  test(`navigate with extension (${mode})`, async ({
    ...
    const confirmationPagePromise = browserContext.waitForEvent('page', (page) => {
      return page
        .url()
        .startsWith('chrome-extension://jakfalbnbhgkpmoaakfflhflbfpkailf/connect.html');
    });
  ...
  test(`snapshot of an existing page (${mode})`, async ({
    browserWithExtension,
    startClient,
    server,
  }) => {
  ...
  test(`extension not installed timeout (${mode})`, async ({
    browserWithExtension,
    startClient,
    server,
  ...
  testWithOldExtensionVersion(
    `works with old extension version (${mode})`,
    async ({ browserWithExtension, startClient, server, useShortConnectionTimeout }) => {
  ...
  test(`extension needs update (${mode})`, async ({
    browserWithExtension,
    startClient,
    server,
    useShortConnectionTimeout,
    overrideProtocolVersion,
  }) => {
    useShortConnectionTimeout(500);
    ...
    await expect(confirmationPage.locator('.status-banner')).toContainText(
      `Playwright MCP version trying to connect requires newer extension version`
    );
...
test(`custom executablePath`, async ({ startClient, server, useShortConnectionTimeout }) => {
  ...
  await fs.promises.writeFile(
    executablePath,
    '#!/bin/bash\necho "Custom exec args: $@" > "$(dirname "$0")/output.txt"',
    { mode: 0o755 }
  );
  ...
  expect(await fs.promises.readFile(test.info().outputPath('output.txt'), 'utf8')).toContain(
    'Custom exec args: chrome-extension://jakfalbnbhgkpmoaakfflhflbfpkailf/connect.html?'
  );
});

test(`bypass connection dialog with token`, async ({
  browserWithExtension,
  startClient,
  server,
}) => {
